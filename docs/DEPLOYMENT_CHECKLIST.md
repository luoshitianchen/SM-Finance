# SM-Finance 上线检查清单（Deployment Checklist）
...
- [ ] 数据库连接指向正确环境（sm_finance）
- [ ] 资源 requests/limits 经压测验证
- [ ] 探针路径 /health、/readyz 可用

## 3. 部署步骤

1. 合并代码，CI 构建镜像并推送 ghcr.io/luoshitianchen/sm-finance
2. 在 staging 部署并完成回归
3. 变更窗口内，ArgoCD 同步 prod（helm/sm-finance，values.yaml + values-prod.yaml）
4. 观察 rollout：`kubectl -n sm-prod rollout status deploy/sm-finance`
...
- 步骤：ArgoCD 回退历史版本 或 `kubectl -n sm-prod rollout undo deploy/sm-finance`。
